                    ...
                    <div class="card">
                        ...
                        <div class="card-body">
                            <form id="create-exercise-form" data-id="create-exercise-form" class="wizard-content mt-2"
                                  action="{{ route('exercises.pp2d.store', $package->id) }}" method="POST">
                                @csrf
                                <input type="hidden" name="package_id" id="package_id" value="{{ $package->id }}">
                                <div id="stepper" class="bs-stepper">
                                    <!-- Header -->
                                    @include('packages.exercises.categories.PP2D.wizards.header')
                                    <!-- Content -->
                                    @include('packages.exercises.categories.PP2D.wizards.content')
                                </div>
                            </form>
                        </div>
                    </div>
