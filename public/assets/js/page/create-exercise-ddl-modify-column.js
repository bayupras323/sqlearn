...
const generateCheckboxColumn = (container, data) => {
    ...
    // create table element
    const table = $('<table></table>').addClass('table table-sm table-bordered').appendTo(responsive);
    const headers = ['', 'Nama Kolom', 'Tipe Data', 'Ubah menjadi ...']
    ...
    data[0].columns.forEach(row => {
        ...
        const selectType = $('<select>', {
            class: 'form-control form-control-sm select2 data-type',
            name: 'dataType[]',
            disabled: true
        }).append(
            $('<option>', {
            ...
            options)
        // column size field
        const columnSize = $('<input>', {
            type: 'number',
            class: 'form-control ml-2 column-size',
            name: 'columnSize[]',
            placeholder: 'Size kolom',
            disabled: true
        });
        $('<td></td>', {class: 'd-flex justify-content-between'}).append(selectType, columnSize).appendTo(tr);
